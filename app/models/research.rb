...
class Research < ApplicationRecord
  ...
  def self.filter(options={})
    ...
    if options[:date_start].present? && options[:date_end].present?
      x = x.with_translations(I18n.locale).where(['research_translations.published_at between ? and ?', options[:date_start], options[:date_end]])

    elsif options[:date_start].present?
      x = x.with_translations(I18n.locale).where('research_translations.published_at >= ?', options[:date_start])

    elsif options[:date_end].present?
      x = x.with_translations(I18n.locale).where('research_translations.published_at <= ?', options[:date_end])
    end
