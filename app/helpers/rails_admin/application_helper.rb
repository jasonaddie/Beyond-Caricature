...
module RailsAdmin
  module ApplicationHelper
    ...
    # get the changelog content rendered as html
    def get_changelog_as_html
    ...
    def get_changelog_current_version_text
      file = Rails.root.join("CHANGELOG.md")
      cache_key = [file, 'version', File.mtime(file)].join('-')

      version_html = Rails.cache.fetch(cache_key, expires_in: 12.hours) do
        heading = nil
        html = ''

        # get the value of the first ## that is not [Unreleased]
        File.open(file).each do |line|
          if line.start_with?('## [') && !line.start_with?('## [Unreleased]')
            heading = line.gsub('##', '').chomp.strip
            break
          end
        end

        if heading
          content_items = heading.split(' - ')
          if content_items.length > 1
            html << I18n.t('admin.actions.changelog.version')
            html << ': '
            html << content_items[0].gsub('[', '').gsub(']', '')
            html << ' ('
            html << content_items[1]
            html << ')'
          end
        end
    ...
    def get_changelog_current_version_link
      link_to({ action: 'changelog', controller: 'rails_admin/main' }, class: "pjax") do
        get_changelog_current_version_text
      end
    end
  end
end
