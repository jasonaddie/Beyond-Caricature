...
module ImportExportLocales
  ...
  class TranslationsExporter
    ...
    def self.export(export_file_prefix, locale_file_prefix='', export_to=nil)
      exporter = self.new(export_file_prefix, locale_file_prefix, export_to)
      exporter.export_translations
    end

    def initialize(export_file_prefix, locale_file_prefix='', export_to=nil)
puts "===="
      ...
      @locale_file_prefix = locale_file_prefix.length > 0 ? "#{locale_file_prefix}." : ''
puts @locales_folder
puts @locale_file_prefix
      @base_yml_file = File.join(@locales_folder, "#{@locale_file_prefix}#{@base_locale}.yml")
puts @base_yml_file
...
#      @cache = YAML.load_file(@duplicates_file)
      ...
      @available_locales.each do |code|
        file_name = "#{@locale_file_prefix}#{code}"
        source_yml = File.join(@locales_folder, "#{file_name}.yml")
        dest_csv = File.join(@export_folder, "#{@prefix}.#{file_name}.csv")
        @foreign_languages << {code: code, yml: source_yml, csv: dest_csv}
      end
puts @foreign_languages
    end
